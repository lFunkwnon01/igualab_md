...
Visión de solución y decisión de base de datos (según presupuesto). **Numerado oficial:** CU001–CU008 y RF-001…021 de [[22 Análisis y Diseño (Borrador) — Estado y Hallazgos|Análisis y Diseño v1.1]] (acta 4: RBAC de 2 roles). Complementa [[06 Arquitectura (AWS)]] y [[08 Ingesta de Datos]].

> ⚠️ **Restricciones de despliegue (actas):** en **desarrollo**, el servidor y el LLM los provee la **universidad** (LLM por API — acta 2). ⬅️ **Acta 4 (REQ-11): la fase 1 NO tendrá despliegue a producción** — el cliente no tiene presupuesto para sostener la infraestructura IA/RAG. Los servicios gestionados de AWS (Cognito, Lambda, Bedrock, OpenSearch) definen la **arquitectura lógica de referencia**; la implementación concreta se ajustará al entorno real. Alternativa ligera ya evaluada: Next.js + FastAPI + LangChain + Supabase (pgvector), ver `Propuesta_Tecnica_Chatbot_Corporativo (1).md`.
