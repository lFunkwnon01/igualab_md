...
| **Consumo del free tier (RN-025/RNF-11)** | 1 llamada LLM por pregunta | 1 llamada por *paso* del agente (la misma pregunta puede consumir 3–6 llamadas) | Con 200 req/día el enfoque agéntico quema el cupo de las demos |
...
| **Capacidad de responder** | Todo lo que está en chunks + lo que ya está en tablas (brechas/sanciones se consultan **vía endpoints REST del frontend**, no vía LLM) | Además, "acciones": el asistente podría *crear* el reporte desde el chat | En fase 1 **no se necesita** que el LLM actúe: CU007 se hace desde la UI con estados ya validados (RN-019) |
| **Costo de ciertos datos estructurados** | Ya resuelto: la UI consulta la BD directamente (REST) y la base persistence el análisis (RN-020) | Redundante con MCP: tool `listar_brechas_gri` duplica lo que ya hace un GET | La BD + REST + vistas ya cubren los datos estructurados |
...
3. **Presupuesto zero-cost apretado**: 1 llamada por pregunta + embeddings locales dejan la mayor parte de los 200 req/día para las demos con Oscar (2 reuniones/semana). El agéntico triplicaría el consumo en el peor caso.
...
|---|---|
| El PO pide que el hable al asistente y **reserve/generar reporte desde el chat** | tool `crear_reporte` (con las confirmaciones que pone RN-019) |
...
| API FastAPI (RBAC RNF-05, pydantic, errores RN-024/025) | ✔ igual | |
| Servicios monolito modular (7 servicios) | ✔ igual | |
| Pipeline de ingesta síncrona (Guard→Parseo→Chunking→Embeddings→Persistencia pgvector HNSW) | ✔ igual | RN-009…RN-016 |
...
| LLM OpenRouter free (GLM-5.2, respaldos) + Embeddings local (server universidad) | ✔ igual | sin function calling |
