...
> Cuestionamiento legítimo del equipo: "¿son necesarias todas las tablas?". Respuesta corta: **sí, y ahora son menos**: la arquitectura por capas quitó módulos y el modelo quedó en **2 bases con 13 tablas** (transaccional 12 + vectorial 1) — cada una sigue respondiendo a la cadena del negocio, a una regla de negocio (RN) o a un propietario distinto del dato (regla oro: nunca dos tablas contienen lo mismo).

## ✅ v3 · RESPUESTA ALINEADA AL MODELO DE DATOS

1. **El modelo son 2 bases**: transaccional (12 tablas) + vectorial (1 tabla: `fragmentos_documento`) = **13 tablas**; sin claves foráneas físicas entre bases (referencias lógicas UUID validadas por la app).
2. **Regla de diseño intacta**: cada tabla tiene un propietario único del dato — nunca dos tablas contienen lo mismo.
3. **Autenticación y asistente (4)**: `sesiones` · `tokens_recuperacion` (autenticación sessionizada — Capa 6: `Sesión (TokenRecuperación)`) · `consultas_asistente` + `consulta_citas` (el Asistente IA — RAG persiste consulta + citas · fuentes).
4. **El propósito de cada tabla no cambia**: la justificación por cadena de negocio + RN sigue vigente.

## 1. Principio de diseño que gobierna el fundamento

BD de fase 1 = **2 bases PostgreSQL 16 + pgvector** (transaccional 12 + vectorial 1 = **13 tablas** · monolito modular), organizadas en 4 grupos — eso responde a la sensación de «son muchas»:
...
| **N2 · Cumplimiento/trazabilidad (1)** | `auditoria` | Obligatoria (append-only) — **absorbe** la traza de los cambios de estado en su campo detalle JSONB. |
...
5. **`gri_analisis`** — *de dónde nace*: RN-027/020 (el humano confirma; todo se registra). *Qué hace*: **la tabla del análisis** — fila por documento + código GRI (empresa y año se derivan del doc); única tabla de resultados (UNIQUE doc + código). Se puebla **automáticamente al terminar la ingesta** y el estado lo asigna **manualmente el Administrador** (`validado_por`) — CU006. El reporte solo lee **esta tabla** con estados validados (RN-041). *Si no existiera*: el reporte tendría que re-analizar PDF en vuelo con el LLM — justo lo prohibido.
   - El **historial de cambios de estado** vive en `auditoria` (evento `AJUSTE_BRECHA` con detalle JSONB: analisis_id, anterior→nuevo, observación).
...
12. **`consultas_asistente`** — *de dónde nace*: el Asistente IA — RAG persiste cada consulta con el modelo usado. *Qué hace*: además del historial de chat, es el **medidor del free tier**: COUNT por día/modelo vs env var `LLM_DAILY_LIMIT` (RNF-027) sin perder trazabilidad (los tokens de embeddings se mide en fase 2 si se requiere, con `uso_embeddings`).
...
## 3. Consolidación v3 — capacidades fusionadas en el dueño del dato (y no sobraban)

| Fusión | Respuesta |
|---|---|
| Gestión de roles | El conjunto cerrado es una regla, no un dato: ENUM/CHECK + índice único parcial en `usuarios.rol` (transferencia de rol = transacción). El catálogo no aporta integridad adicional en fase 1 |
| `configuracion` | El módulo Configuración fue quitado en fase 1 → umbrales vía variables de entorno (doc Stack §4) |
| Medidor LLM separado | El consumo diario se deriva de `consultas_asistente` (COUNT por día/modelo) — una tabla, dos de sus usos: historial + medidor |
| Histórico GRI separado | El detalle JSONB de los eventos `AJUSTE_BRECHA` de `auditoria` cubre la traza por fila (anterior→nuevo, observación, quien) |
...
Estas fusiones respondieron de una vez a las candidatas plausibles: una capacidad retirada no cayó «por si acaso», cada retirada conservó su fundamento dentro del dueño del dato.
...
> Cada tabla existe por un hecho de negocio (actas/plan/RN) y un dueño único del dato. Ahora son **13** (2 bases): las capacidades fusionadas cambiaron la forma del modelo, no la regla de diseño.
