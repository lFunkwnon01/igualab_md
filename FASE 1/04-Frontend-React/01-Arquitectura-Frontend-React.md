...
| `/#/auditoria` | Superadmin | tabla filtrable (usuario/fecha/tipo), solo lectura | RN-042/047/048 |
| `/#/ia` | Administrador | chat con citación (doc + sección) y aviso de cuota/no disponible | RF-023/031/049 |
| `/#/reportes` | Administrador | selección empresa/año + listado de brechas validadas → Generar PDF | RF-025/038/043 |
| `/#/descargas` | Administrador | historial y descarga de versiones | RF-026/045 |
...
3. **Chat con citas (RF-023/031)**: los mensajes del asistente renderizan **chips de cita** (doc_id·sección) que abren el fragmento citado; si no hay fuente, el sistema muestra el mensaje de «sin información» tal como queda acordado (RN-033).
...
5. **Validación de estados antes del reporte**: si quedan brechas sin confirmar, el botón Generar queda deshabilitado y hace el vínculo con el detalle faltante (RF-019/035).
